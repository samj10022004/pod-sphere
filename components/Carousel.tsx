...
const EmblaCarousel: React.FC<PropType> = ({fansLikeDetail}:CarouselProps) => {
        ...
        <div className="flex">
            {slides.slice(0,5).map((item)=>(
                <figure key={item._id} className='carousel_box' onClick={()=>router.push(`/podcast/${item.podcast[0]?.podcastId}`)}>
                    <Image src={item.imageUrl} className='absolute size-full rounded-xl border-none' alt='card' fill/>
